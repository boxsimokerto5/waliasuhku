...
export default function InitialAssessmentModal({ child, onClose, onSave }: InitialAssessmentModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4 box-border max-w-full overflow-x-hidden">
      <motion.div
        initial={{ opacity: 0, scale: 0.95, y: 15 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.95, y: 15 }}
        className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-[95%] sm:max-w-4xl max-h-[92dvh] sm:max-h-[92vh] my-auto flex flex-col overflow-hidden text-slate-800 border border-slate-100 min-w-0 box-border"
      >
        {/* Header Block */}
