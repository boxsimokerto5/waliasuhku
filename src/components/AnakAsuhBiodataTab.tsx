...
export default function AnakAsuhBiodataTab({
  ...
  const [subTab, setSubTab] = useState<'biodata' | 'portofolio' | 'laporan' | 'galeri'>('biodata');
        ...
        {subTab !== 'galeri' && (
          <button
            ...
            onClick={async () => {
              ...
              try {
                if (subTab === 'laporan') {
                  await generateStudentMonthlyReportPDF(currentUser, users);
                } else {
          ...
          >
            ...
            <span>{isPrinting ? 'Mencetak...' : subTab === 'laporan' ? 'Unduh Laporan Bulanku (A4)' : 'Unduh Portofolioku (A4)'}</span>
          </button>
        )}
      </div>
        ...
        {subTab === 'laporan' && (
          <div className="space-y-6">
            <div className="bg-rose-50/50 p-4 rounded-2xl border border-rose-100 flex items-start gap-3">
              <Heart className="w-5 h-5 text-rose-500 shrink-0 mt-0.5 fill-rose-500 animate-pulse" />
              <div>
                <h4 className="text-xs font-black text-rose-800 uppercase tracking-wider">
                  Laporan Perkembangan Bulanku
                </h4>
                <p className="text-[11px] text-rose-600/90 mt-1 leading-relaxed font-medium">
                  Halaman ini menampilkan dokumentasi perkembangan bulanan Anda yang disusun oleh Wali Asuh untuk dikirimkan kepada orang tua sebagai pertanggungjawaban asrama.
                </p>
                <div className="mt-3 p-2.5 bg-white/80 border border-rose-100 rounded-xl text-[10px] text-slate-500 space-y-1 font-medium">
                  <p className="text-slate-650 font-bold">📢 Keterbukaan & Privasi:</p>
                  <p>• Data ini bersifat rahasia asrama dan keluarga Anda.</p>
                  <p>• Sesuai kebijakan privasi, informasi saldo tabungan dan hafalan tahfidz ditiadakan dari cetakan laporan bulanan ini.</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-slate-50/50 border border-slate-100 rounded-2xl p-4 space-y-3">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">I. Kondisi Kesehatan</span>
                <div>
                  <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-md text-[8.5px] font-black uppercase">
                    {currentUser.healthStatus || "Sangat Sehat"}
                  </span>
                </div>
                <p className="text-xs text-slate-600 leading-relaxed font-medium">
                  {currentUser.healthNotes || "Siswa dalam kondisi sehat walafiat, aktif mengikuti seluruh kegiatan."}
                </p>
              </div>

              {/* Card Kegiatan */}
              <div className="bg-slate-50/50 border border-slate-100 rounded-2xl p-4 space-y-3">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">II. Kegiatan & Pembinaan</span>
                <p className="text-xs text-slate-600 leading-relaxed font-medium">
                  {currentUser.monthlyActivities || "Aktif mengikuti seluruh rangkaian ibadah wajib, kajian malam, bimbingan belajar, dan olahraga."}
                </p>
              </div>

              {/* Card Karakter */}
              <div className="bg-slate-50/50 border border-slate-100 rounded-2xl p-4 space-y-3">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">III. Catatan Perkembangan Karakter</span>
                <p className="text-xs text-slate-600 leading-relaxed font-medium">
                  {currentUser.characterNotes || "Menunjukkan adab yang baik, sopan santun kepada pembina asrama, serta disiplin melaksanakan piket harian."}
                </p>
              </div>
            </div>

            <div className="p-4 bg-slate-50 border border-slate-150 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 text-center sm:text-left">
              <div>
                <p className="text-xs font-black text-slate-800">Butuh Cetakan Fisik Resmi?</p>
                <p className="text-[10px] text-slate-500 mt-0.5 font-medium">Unduh dokumen laporan ini dalam format PDF A4 resmi siap cetak.</p>
              </div>
              <button
                type="button"
                onClick={async () => {
              ...
              >
                ...
                <span>{isPrinting ? 'Mencetak...' : 'Unduh Laporan Bulanku (PDF)'}</span>
              </button>
            </div>
          </div>
        )}
        ...
        {subTab === 'galeri' && (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {(!currentUser.activityPhotos || currentUser.activityPhotos.length === 0) ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {currentUser.activityPhotos.map((photo) => (
